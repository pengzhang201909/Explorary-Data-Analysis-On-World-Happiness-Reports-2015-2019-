...
### Question 2: How does democracy affect country happiness? (MICHAEL'S CONTRIBUTION)

To measure a country's democracy level, we'll use the data from https://www.gapminder.org/data/documentation/democracy-index/, which assigns each country a democracy index. The more democratic freedoms a country has, the higher their democracy index.
...
This data table only provides data up to year 2018, so we'll exclude the year 2019 from our analysis.
...
Our 2 datasets are tidy and are ready to merge.
...
Let's visualize the relationship between the democracy index and happiness to spot any patterns.
...
The quadratic regression appears to fit greatly, but we must make sure that our residual plots don't exhibit any patterns, and the residuals should be distributed approximately normal around 0. The formula for a residual is (actual - expected).
...
Now that we calculated our residua
...
ggplot(data) + 
  ...
  facet_wrap(~year) + 
  theme_bw() + 
  labs(x="Residuals",y="Frequency",title="Residual Distribution",caption="Figure 2.6 Distribution of Residuals")
```

Every year's residual distribution is approximately normal and centered around 0.
...
ggplot(data) + 
  ...
  facet_wrap(~year) + 
  theme_bw() + 
  labs(title="Residual Plots",x="Democracy Index",y="Residuals",caption="Figure 2.7 Residual Plots")
```

The residual plots show no pattern, so our model is a good predictor.



Should I add more? I can explore this interesting graph:

```{r}
data %>%
  ggplot() + 
  geom_point(aes(DemocracyIndex,trust,color=year),size=0.9,alpha=7/8) +
  facet_wrap(~year)
```
...
#a[2] = curve steepness
#a[3] = midpoint
sigmoid_model <- function(a, data){a[1]/ (1+ (exp (-a[2]* (data$DemocracyIndex-a[3]) ) ) )}

measure_distance <- function(model, data) {
  difference <- data$score - sigmoid_model(model, data)
  sqrt(mean(difference ^ 2))
}

best2015 <- optim(c(5, 50, 70), measure_distance, data = data2015)
best2016 <- optim(c(5, 50, 70), measure_distance, data = data2016)
best2017 <- optim(c(5, 50, 70), measure_distance, data = data2017)
best2018 <- optim(c(5, 50, 70), measure_distance, data = data2018)

function2015 <- function(x){ best2015$par[1] / (1+ (exp (-best2015$par[2]* (x - best2015$par[3]) ) ) )}
function2016 <- function(x){ best2016$par[1] / (1+ (exp (-best2016$par[2]* (x - best2016$par[3]) ) ) )}
function2017 <- function(x){ best2017$par[1] / (1+ (exp (-best2017$par[2]* (x - best2017$par[3]) ) ) )}
function2018 <- function(x){ best2018$par[1] / (1+ (exp (-best2018$par[2]* (x - best2018$par[3]) ) ) )}
