...
There is a interesting trend in Figure 3.4. When a country had happiness score more than 5 which meant this was a happy country(Figure 2.1), democracy had strongly positive relationship with happiness. Hong Kong had happiness score 5.4 in 2018! But when the country has a score below 5, it appears that democracy has no relationship with a country's happiness.
...
We all know that a complete and accurate data source is important and is fundamental to data analysis. That's why we used the 4E rules to do the data cleaning job. In the exciting exploratory analysis journey, we can see that the world is mostly a happy place for people, but the results were only based on a survey of a sample of 1000 people for each country. Since every country has a different amount of population, it would be more accurate if a weight was assigned for each country for calculating the world's happiness score. For future research, someone could assign an appropriate weight for each country, and re-calculate the world's happiness score.

From our analysis between a country's democracy and its happiness score, a country tends to be happier when it has more democracy, especially for countries that have a happiness score greater than 5.
