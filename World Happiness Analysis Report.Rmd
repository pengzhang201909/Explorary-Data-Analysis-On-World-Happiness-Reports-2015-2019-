...
w_18 <- read.csv("2018.csv",stringsAsFactors = FALSE)
...
```{r}
w_18 <- w_18 %>% dplyr::filter(country != "United Arab Emirates")
...
```{r}
w <- w %>% group_by(country) %>% mutate(count = sum(year)) %>% dplyr::filter(count == sum(2015:2019))
...
The geom_line graph of happiness score. We can not find too much information from this graph.
```{r}
w %>% 
...
The geom_line of predictions, we can see from 2015 to 2016 the prediction is increasing.
```{r message=FALSE}
preds %>% 
...
Table 2.12 Predicted happiness score for Angola in the year 2015
...
Based on our analysis of a country's amount of democratic freedoms, a country with more democratic freedoms is more likely to be happyier than countries with lower democratic freedoms, especially when the democracy index is higher than 50. 

## Contributions

Alex Zhangpeng - Wrote Introduction, tidied the data, and wrote question 3, and managed project workflow on github.

Michael Chen- Wrote question 2, and the conclusion for question 2, and also managed project workflow on github.
