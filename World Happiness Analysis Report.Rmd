...
### Question 1: HONGYANG'S CONTRIBUTION 
...
data %>%
  ...
  facet_wrap(~year) + 
  labs(x="Democracy Index",y="Happiness Score", title="Democracy Index vs Happiness Score by Year",caption="Figure 2.4 Visualizing the trend between democracy index and happiness score. We can an approximate quadratic regression.") + 
  theme_bw()
```
...
The quadratic regression appears to fit greatly, but we must make sure that our residual plots don't exhibit any patterns, and the residual histogram must be approximately normal. The formula for a residual is (actual - expected).
